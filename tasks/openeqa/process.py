...
def process(cfg):
    ...
    for i, item in enumerate(tqdm.tqdm(dataset)):
        processed_item = {
            "question_id": item["id"],
            "img_path": [osp.join('/home/vlm/eval_images/', i)  for i in item['images']] if type(item['images']) == list else  osp.join('/home/vlm/eval_images/', item['images']),
            "question": item["question"],
            "answer":  item["answer"],
            "question_type": 'short-answer',
            "extra_answer":  item['extra_answer'],
            "type_level_1": item["type"].split('_')[-1],
            "type_level_2": '_'.join(item["type"].split('_')[:2])
        }
