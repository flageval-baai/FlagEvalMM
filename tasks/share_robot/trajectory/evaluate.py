...
def extract_points(point_str, width, height):
    """
    Extracts coordinate points from string input, normalizes them by width and height if needed.
    Supports various input formats and ignores line-end comments.

    Args:
        point_str: String representation of points
    ...
    # Method 1: Try extracting full list format
    ...
    # Method 2: Extract line-by-line
    points = []
    for line in point_str.split("\n"):
        clean_line = re.sub(
            r"#.*$", "", line
        ).strip()  # Remove comments and trailing spaces
        if match := re.search(
            r"\[([-+]?\d*\.?\d+)\s*,\s*([-+]?\d*\.?\d+)\]", clean_line
        ):
...
def discrete_frechet_distance(P, Q):
    """Compute the discrete Fréchet distance between two point sets"""
    n = len(P)
    m = len(Q)
    ca = np.zeros((n, m))
    ca.fill(-1.0)
    ...
    def compute_ca(i, j):
        ...
        if i == 0 and j == 0:
            ...
            ca[i, j] = max(
                min(
                    compute_ca(i - 1, j), compute_ca(i - 1, j - 1), compute_ca(i, j - 1)
                ),
                dist_matrix[i, j],
            )
        return ca[i, j]
...
def root_mean_square_error(P, Q):
    """Compute the Root Mean Square Error (RMSE) between two point sets"""
    min_len = min(len(P), len(Q))
    if min_len == 0:
        return 0.0
    squared_distances = [
        np.sum((np.array(P[i]) - np.array(Q[i])) ** 2) for i in range(min_len)
    ]
...
def calculate_metrics(data):
    """
    Calculate DFD, HD, and RMSE metrics for each item in the input list.

    Args:
        data: A list of dictionaries, each with 'answer', 'gt', 'width', and 'height' keys.

    Returns:
        A dictionary with average values of each metric.
    """
    dfd_list = []
    hd_list = []
    rmse_list = []
...
def get_result(annotations: Dict, predictions: List[Dict]) -> Dict:
    """
    Evaluate model predictions using various geometric metrics.
    ...
    Returns:
        Dict: Dictionary containing average evaluation metrics
    """
    result = []
