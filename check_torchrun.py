...
def main():
    ...
    try:
        ...
        print(f"Rank: {rank}, Local Rank: {local_rank}, World Size: {world_size}, Master Addr: {master_addr}, Master Port: {master_port}")
    except KeyError as e:
        print(f"Error: 环境变量缺失 {e}. 请确保使用 'torchrun' 启动此脚本。")
        return

    print(f"[Init] Rank {rank}/{world_size} (Local {local_rank}) on host {socket.gethostname()} starting...")
    ...
    if torch.cuda.is_available():
        ...
        print(f"[Device] Rank {rank} is using GPU: {torch.cuda.get_device_name(local_rank)}")
    else:
    ...
    if result == world_size:
        ...
        print(f"[Fail] Rank {rank} check FAILED! Result: {result}, Expected: {world_size}")
