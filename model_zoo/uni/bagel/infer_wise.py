...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="JSON file containing lines of metadata for each prompt.",
    )
    ...
    print(
        f"GPU {rank}: Processing {end - start} prompts (indices {start} to {end - 1})"
    )
    ...
    for idx in range(start, end):
        ...
        print(
            f"GPU {rank} processing prompt {idx - start + 1}/{end - start}: '{prompt}'"
        )
